...
\begin{frame}[fragile]
\frametitle{QT Resources}
\end{frame}

\subsection{MP3 Player}
\begin{frame}[fragile]
\frametitle{MP3 Player}
Create an application with the following graphical user interface

\begin{itemize}
\item Load Button: the user could load an mp3 file. The selected file will be displayed
in the status bar.
\item Play Button: the selected song will be played. If a song is played, the play
button will be changed into a stop button.
\item Stop Button: the currently played song will be stopped.
\item Exit Menu: the application will be closed.
\end{itemize}
...
{\small
\begin{enumerate}
\item Set the minimum Size of the Widget to x=800, y=460
\item Overload the paintEvent method and iterate over all pixels of the widget. 
\item Implement a method calcMandelbrotPoint that accepts the x-coordinate (int x) and y-coordinate (int y) and a recusrsion limit (int maxIterations=200)
	\begin{itemize}
	\item Scale the coordinate x linear to the numeric range from -2.5 to 1.0 (Save the scaled value in x1)
	\item Scale the coordinate y linear to the numeric range from -1.0 to 1.0 (Save the scaled value in y1)
	\item Initialize double x0 = 0.0 and y0 = 0.0
	\item Loop as long $({x_0}^2 + {y_0}^2) < 4.0$ and as long the iteration counter is not exceeding maxIterations.
	\item Calculate Real Number $x_0 = {x_0}^2 - {y_0}^2 + x_1$ (only replace the x0 variable value after the next operation)
	\item Calculate Imaginary Number $y_0 = 2 \cdot  x_0 \cdot y_0 + y_1$ 
...
\begin{frame}[fragile]
\frametitle{Network Connections}
\end{frame}
